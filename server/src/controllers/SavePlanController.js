...
class SavePlanController {
	async save(req, res) {
		...
		for (let i = 1; i <= 30; i++) {
			...
			if (i == 29) {
				worksheet.column(i).setWidth(5);
			}
			if (i == 30) {
				worksheet.column(i).setWidth(7);
			}
		}
		...
		worksheet
			.cell(2, 2, 9, 2, true)
			.string('Назви навчальних дисциплін')
			.style(myStyle);
		worksheet
			.cell(2, 3, 2, 5, true)
			.string('Розподіл контрольних заходів за семестрами')
			.style(myStyle);
		...
		worksheet
			.cell(3, 5, 9, 5, true)
			.string('Індивідуальні завдання')
			.style(myStyle);

		worksheet
			.cell(2, 6, 9, 6, true)
			.string('Кількість кредитів ЄКТС')
			.style(myStyle);

		worksheet
			.cell(2, 7, 2, 12, true)
			.string('Кількість годин')
			.style(myStyle);
		worksheet
			.cell(3, 7, 9, 7, true)
		...
		worksheet
			.cell(6, 13, 6, 28, true)
			.string('Модульні атестаційні цикли')
			.style(myStyle);
		...
		worksheet
			.cell(8, 13, 8, 28, true)
			.string('Кількість тижнів у модульному атестаційному циклі')
			.style(myStyle);
		...
		Categories.forEach(function(item, i, categories) {
			worksheet
				.cell(10 + lineIter + 1, 1, 10 + lineIter + 1, 30, true)
				.string(i + 1 + '.' + categories[i].dataValues.name)
				.style(myStyle); // Категорії
			lineIter++;
			categories[i].dataValues.sub_categories.forEach(function(
				item,
				j,
				sub_categories,
			) {
				worksheet
					.cell(11 + lineIter, 1, 11 + lineIter, 30, true)
					.string(
						i + 1 + '.' + (j + 1) + ' ' + sub_categories[j].dataValues.name,
					)
					.style(myStyle); // Підкатегорії
				lineIter++;
				educationItems.forEach(function(item, e, educationItem) {
					if (
						sub_categories[j].dataValues.sub_category_id ==
						educationItem[e].dataValues.sub_category_id
					) {
						let distribution_of_hours = 0;
						let educationPlanDepartmentName;
						educationItem[e].dataValues.distribution_of_hours.forEach(function(
							item,
							d,
							distribution_of_hour,
						) {
							distribution_of_hours += distribution_of_hour[d].dataValues.value;
							worksheet
								.cell(
									11 + lineIter,
									12 + distribution_of_hour[d].dataValues.module_number,
								)
								.number(distribution_of_hour[d].dataValues.value)
								.style(myStyle); // Години

							//Расчёт сумм часов
							Array_Module.push(
								distribution_of_hour[d].dataValues.module_number,
							);
						...
						educationPlans.forEach(function(item, p, educationPlan) {
							if (
								educationPlan[p].dataValues.id ==
								educationItem[e].dataValues.education_plans_id
							) {
								educationPlanDepartmentName =
									educationPlan[p].dataValues.department.dataValues.name;
							}
						});
						...
						worksheet
							.cell(11 + lineIter, 2)
							.string(educationItem[e].dataValues.subject.dataValues.name)
							.style(myStyle); // Назви навчальних дисциплін
						educationItem[e].dataValues.distribution_of_controls.forEach(
							function(item, r, distribution_of_control) {
								worksheet
									.cell(11 + lineIter, 3)
									.string(distribution_of_control[r].dataValues.exams)
									.style(myStyle); // Екзамени
								worksheet
									.cell(11 + lineIter, 4)
									.string(distribution_of_control[r].dataValues.credit)
									.style(myStyle); // Заліки
								worksheet
									.cell(11 + lineIter, 5)
									.string(distribution_of_control[r].dataValues.exams)
									.style(myStyle); //Індивідуальні завдання
							},
						);
						worksheet
							.cell(11 + lineIter, 6)
							.number(educationItem[e].dataValues.credits)
							.style(myStyle); // Кредити
						worksheet
							.cell(11 + lineIter, 7)
							.number(educationItem[e].dataValues.credits * 30)
							.style(myStyle); // загальний обсяг
						worksheet
							.cell(11 + lineIter, 8)
							.number(distribution_of_hours * 8)
							.style(myStyle); // всього
						distribution_of_hours_all += distribution_of_hours * 8;
						worksheet
							.cell(11 + lineIter, 9)
							.number(educationItem[e].dataValues.lectures)
							.style(myStyle); // Лекції
						...
						worksheet
							...
							.style(myStyle); // практичні, семінарські
						practices_all +=
							distribution_of_hours * 8 - educationItem[e].dataValues.lectures;
						worksheet
							.cell(11 + lineIter, 11)
							.number(educationItem[e].dataValues.laboratories)
							.style(myStyle); // лабораторні
						...
						worksheet
							...
							.style(myStyle); // самостійна робота
						independent_work +=
							educationItem[e].dataValues.credits * 30 -
							distribution_of_hours * 8;
						worksheet
							.cell(11 + lineIter, 29)
							.string(educationPlanDepartmentName)
							.style(myStyle); // Кафедра викладання
						lineIter++;
					}
				});
				...
				worksheet
					.cell(11 + lineIter, 8)
					.number(distribution_of_hours_all)
					.style(myStyle); // Всього
				...
				worksheet
					.cell(11 + lineIter, 9)
					.number(lectures_all)
					.style(myStyle); // Всього лекцій
				lectures_allSubCategory += lectures_all;
				lectures_all = 0;
				worksheet
					.cell(11 + lineIter, 10)
					.number(practices_all)
					.style(myStyle); // Вього практичних, семінарських
				practices_allSubCategory += practices_all;
				practices_all = 0;
				worksheet
					.cell(11 + lineIter, 11)
					.number(laboratores_all)
					.style(myStyle); // Всього лабораторних
				laboratores_allSubCategory += laboratores_all;
				laboratores_all = 0;
				worksheet
					.cell(11 + lineIter, 12)
					.number(independent_work)
					.style(myStyle); // Всього самостійна робота
				independent_work_allSubCategory += independent_work;
				independent_work = 0;

				for (let j = 0; j < result.length; j++) {
					worksheet
						.cell(11 + lineIter, 12 + result[j].m_num)
						.number(result[j].hours)
						.style(myStyle); //Подсчёт часов
					...
					for (let i = 0; i < Array_Module_allSubCategory.length; i++) {
						FullArray_allSubCategory[i] = Array.of(
							Array_Module_allSubCategory[i],
							Array_Value_allSubCategory[i],
						);
					}
			...
			for (let j = 0; j < result_allSubCategory.length; j++) {
				worksheet
					.cell(11 + lineIter, 12 + result_allSubCategory[j].m_num)
					.number(result_allSubCategory[j].hours)
					.style(myStyle);
				...
				for (let i = 0; i < Array_Module_total.length; i++) {
					FullArray_total[i] = Array.of(
						Array_Module_total[i],
						Array_Value_total[i],
					);
				}
		...
		worksheet
			.cell(11 + lineIter, 8)
			.number(distribution_of_hours_total)
			.style(myStyle);
		worksheet
			.cell(11 + lineIter, 9)
			.number(lectures_total)
			.style(myStyle);
		worksheet
			.cell(11 + lineIter, 10)
			.number(practices_total)
			.style(myStyle);
		worksheet
			.cell(11 + lineIter, 11)
			.number(laboratores_total)
			.style(myStyle);
		worksheet
			.cell(11 + lineIter, 12)
			.number(independent_work_total)
			.style(myStyle);
		lineIter++;
		worksheet
			.cell(11 + lineIter, 1)
			...
			.style(myStyle);
		worksheet
			.cell(11 + lineIter, 2, 11 + lineIter, 12, true)
			.string('Кількість годин на тиждень')
			.style(myStyle);
		for (let j = 0; j < result_total.length; j++) {
			worksheet
				.cell(11 + lineIter, 12 + result_total[j].m_num)
				.number(result_total[j].hours)
				.style(myStyle);
		}
		lineIter++;
		worksheet
		...
		worksheet
			.cell(11 + lineIter, 2, 11 + lineIter, 12, true)
			.string('Кількість курсових робіт')
			.style(myStyle);
