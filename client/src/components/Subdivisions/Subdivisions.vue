<template>
  <div>
    <v-toolbar dark color="primary">
      <v-toolbar-title>Підрозділи</v-toolbar-title>
      <v-spacer></v-spacer>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12>
                    <v-text-field v-model="editedItem.name" label="Назва підрозділу" :rules="requiredField"></v-text-field>
                  </v-flex>
                </v-layout>
              </v-container>
            </v-card-text>
  ...
  export default{
    ...
    data(){
      return{
        ...
        headers: [
          { text: 'Назва підрозділу', value: 'name' },
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'Новий підрозділ' : 'Редагувати підрозділ'
      },
      getRequestId(){
